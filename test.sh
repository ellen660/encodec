# CUDA_VISIBLE_DEVICES=1 python3 encodec/train.py --exp_name 091224_l1 &

# CUDA_VISIBLE_DEVICES=2 python3 encodec/train.py --exp_name 091224_l2 &

# python3 encodec/train.py --exp_name config

# python3 encodec/train.py --exp_name 141224_l1 &

# python3 encodec/train.py --exp_name 151224_l1 &

# python3 encodec/train.py --exp_name 161224_l1 &

# python3 encodec/train.py --exp_name 171224_l1 &

# python3 encodec/train.py --exp_name 181224_l1 &

# python3 encodec/train.py --exp_name 191224_l1 &

# python3 encodec/train.py --exp_name 201224_l1 &

# python3 encodec/train.py --exp_name 211224_l1 &

# python3 encodec/train.py --exp_name 221224_l1 &

# python3 encodec/train.py --exp_name 231224_l1 &

# python3 encodec/train.py --exp_name 241224_l1 &

# python3 encodec/train.py --exp_name 251224_l1 &

# python3 encodec/train.py --exp_name 261224_l1 &

python3 encodec/train.py --exp_name 271224_l1 &