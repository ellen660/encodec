    ...
    if fmap_real is not None:
        for tt1 in range(len(fmap_real)):  # len(fmap_real) = num discriminators
            ...
            for tt2 in range(len(fmap_real[tt1])):  # len(fmap_real[tt1]) = 5
                l_feat = l_feat + torch.nn.L1Loss(reduction="mean")(
                    fmap_real[tt1][tt2].detach(), fmap_fake[tt1][tt2]
                ) / torch.mean(torch.abs(fmap_real[tt1][tt2].detach()))
        ...
        KL_scale = len(fmap_real) * len(
            fmap_real[0]
        )  # len(fmap_real) == len(fmap_fake) == len(logits_real) == len(logits_fake) == disc.num_discriminators == K
        l_feat /= KL_scale
...
def disc_loss(logits_real, logits_fake):
    ...
    for tt1 in range(len(logits_real)):  # [1,1, T, filters*2]
        ...
        loss_d = (
            loss_d + torch.mean(logits_fake[tt1] ** 2) + torch.mean((1 - logits_real[tt1]) ** 2)
        )  # DAC squared loss
