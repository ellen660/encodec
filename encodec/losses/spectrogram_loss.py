...
class BreathingSpectrogram(nn.Module):
    def __init__(self, sampling_rate=10, n_fft=256, hop_length=None, win_length=None, device="cuda"):
        """
...
class ReconstructionLoss(nn.Module):
    def __init__(
        self, alpha=0.01, bandwidth=None, sampling_rate=10, n_fft=1024, hop_length=None, win_length=None, device="cuda"
    ):
        super().__init__()
        self.spectrogram = BreathingSpectrogram(
            sampling_rate=sampling_rate, n_fft=n_fft, hop_length=hop_length, win_length=win_length, device=device
        )
...
class ReconstructionLosses(nn.Module):
    def __init__(
        self,
        hop_length: list[float],
        win_length: list[float],
